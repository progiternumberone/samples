...
//		 Modifications:
//
//   	MV -- Modified for AMD (require) compatibility. 	
//		MV -- Added define block			
//		MV -- changed line 502 from self.view.searchList.length to self.view.searchList
//		MV -- changed line 503 changed class attribute to id for storing col name.  Added class attribute to style select with bootstrap (form-control class)
//		MV -- added split on classname in onpagechanged for view to get first class.  Added form-control class to page number input box
//		

define("bbGrid", [ "backbone" , "backbone.syphon", "tpl!common/bbgrid/templates/gridfilter.tpl","jquery"], 
	 		function(Backbone, Backbone_Syphon, gridfilterTpl,$ ){
	
	Backbone.Syphon.KeyExtractors.registerDefault(function($el){
		  return $el.attr("name");  //prop call fix.  
		});	
	
	bbGrid = 
		
	(function () {console.log(Backbone_Syphon);
	    ...
	    viewOptions = ['autofetch', 'buttons', 'actions', 'colModel', 'container',
	        ...
	        'onSubmitFilter', 'enableServerFilter', 'onClearFilter', 'onServerSort',  //Added..
	        'showTooltipOnLargeContent', 'contentMaxLength'];  
	    ...
	    bbGrid.RowView = function (options) {
	        this.events = {
	            ...
	            'click td.bbGrid-multiselect-control > input[type=checkbox]' : 'setCheckBox',
	            'click td[class!=bbGrid-actions-cell]': 'setSelection',
	            'mouseenter div[rel=popover]': 'showTooltip',
	        };
	        ...
	        this.view.showTooltipOnLargeContent = (this.view.showTooltipOnLargeContent == undefined)?  true : this.view.showTooltipOnLargeContent ; //tooltip mv.
	        this.view.contentMaxLength = (this.view.contentMaxLength == undefined)?  50 : this.view.contentMaxLength ; //cell length greater than max will trigger tooltip.
	    };
	    ...
	    _.extend(bbGrid.RowView.prototype, Backbone.View.prototype, {
	        ...
	        template: 
	        	...
	        	(this.showTooltipOnLargeContent == true || this.showTooltipOnLargeContent == undefined)? //Add tooltip else default to original template
	        		_.template(
	        	            '<% if (isMultiselect) {%>\
	        	            ...
	        	            <%} _.each(values, function (cell) { %>\
	        	                <td contenteditable="<%=cell.editable || false%>" <% if (cell.name === "bbGrid-actions-cell") {%>class="bbGrid-actions-cell"<%}%> <% if (cell.width) {%>style="width:<%=cell.width%>"<%}%> >\
	                				<% if (cell.value != null && cell.value.length > ContentMaxLength) {%> <div rel=popover data-toggle=popover data-content="<%=cell.value%>"> <%= cell.value.substring(cell.value, ContentMaxLength) %>  ...	</div><% }else %> \
	        	        			<%=cell.value%>\
	        	                </td>\
	        	...
	        	: _.template(
        	            ...
        	            <%} _.each(values, function (row) { %>\
        	                <td contenteditable="<%=row.editable || false%>" <% if (row.name === "bbGrid-actions-cell") {%>class="bbGrid-actions-cell"<%}%>  <% if (cell.width) {%>style="width:<%=cell.width%>"<%}%>>\
        	        			<%=row.value%>\
        	                </td>\
        	            <%})%>', null, templateSettings)	
	    	...
	    	showTooltip: function(event){
	    		//Fixing issue caused by splitter
	    		$("script[src='assets/js/vendor/jquery-1.4.3.min.js']").remove();
	    		require([ $ ], function() {
		    		var $target = $(event.currentTarget);
		    		$target.popover('show');
		    		$target.parent().mouseleave(function(e){
		    			e.stopPropagation();
		    			$target.popover('hide');
	        		});
	    		});
	        ...
	        onDblClick: function (event) {
	        	if (this.view.multiselect){
		        	this.setCheckBox(event); //automatically check box for easier selection.
	        	}
	        ...
	        setCheckBox: function(event){
	            var options = event || {};
	            var checked = true;
                this.setSelection(options, checked);
	        },

	        setSelection: function (options, checked) {
	            var options = options || {};
	        	...
	        	if (!this.view.multiselect || checked){  //execute block if checkbox is checked and multi-select enabled or just single select.  setSelection triggered on double click too?
	        	...
	        	else if(this.view.multiselect && !checked){   		
	        ...
	        render: function () {
	            ...
	            html = this.template({
	                ...
	                ContentMaxLength: this.view.contentMaxLength, //tooltip mv.
	            ...
	            // Disabled this code due to issue with automatic highlighting of rows on new pages after a row is selected.
	            // if (isChecked) { this.selected = true;  this.$el.addClass('warning'); }


	            this.$el.html(html);
	            
	            this.$el.find('div[rel=popover]').popover({ //tooltip mv.
	            	  trigger: 'manual',   	  
	            	  placement: 'bottom'
	            	});
	    ...
	    bbGrid.PagerView.extend = Backbone.View.extend;
		    ...
		    bbGrid.FilterView2 = function (options) {
		        this.events = {
			            'click .js-submit' : 'onApplyFilter',
			            'click .js-clear' : 'onClearFilter',
			            'click .rdg-sort' : 'onSortFilter',
			            'click .rdg-filter a': 'onShowFilter',
				        'change .operator-dropdown' : 'onOperatorChange',
			    };
		        Backbone.View.apply(this, [options]);
		        this.view = options.view;
		    ...
		    _.extend(bbGrid.FilterView2.prototype, Backbone.View.prototype, {
		        tagName: 'div',
		        template: gridfilterTpl,
		        //className: 'bbGrid-grid-nav row',
		        
		        onShowFilter: function(e){
		        	
		        	var charColumns = ["RequestAction","RecordType"];
		        	var noContainsTypes = ["date","int"];
		        	var $div = $(e.target).parent().parent();
		        	 
		        	/*** Have only Equal and Not equal for Character columns   ***/
		        	if( ( $.inArray(this.model.get("column"), charColumns)) > -1 ) {

		        		$div.find('.operator-dropdown option').filter(function() { 
	            	        return ($(this).attr('data-show') == 'optional'); 
	            	    }).remove();

		        	 }
		        	/*** Remove Contains for Date and Integer columns  ***/
		        	if(( $.inArray(this.model.get("type"), noContainsTypes)) > -1){

		        		 $div.find('.operator-dropdown option').filter(function() { 
	            	        return ($(this).val() == 'contains'); 
	            	    }).remove();
		        	...
		        	e.stopPropagation(); //prevent click event from reaching th
		        	$(e.target).parent().parent().toggleClass("open"); //opens form
		        },
		        onOperatorChange: function(e){
		        	  ...
		        	  //this.model.set("operator", text);
		        	  console.log(text);

			        	if(text == "between"){
			        		$form.find('.rdg-criteria1').hide();
			        		$form.find('.rdg-criteria2').show();
							 ...
							 if( this.model.get("type") == "date" ){
								...
								if ($form.find('.datepicker').length > 0){
									$form.find("#ui-datepicker-div").off("click");
					                
									$form.find('.datepicker').datepicker('destroy');    
									$form.find('.datepicker').removeClass("hasDatepicker").removeAttr('id');    								
								}
	
								$form.find('.datepicker').datepicker({dateFormat: "yy-mm-dd"});
								
				                $("#ui-datepicker-div").on("click", function(event) {  //stop click event from dismissing the filter view.
			                        event.stopPropagation();
			                    });
							 }
			        	}else{
			        		$form.find('.rdg-criteria2').hide();
			        		$form.find('.rdg-criteria1').show();
			        	}
		        },
		        onApplyFilter: function(e){ //mv
		        	e.preventDefault();
		        	...
		        	if(data.operator == "between"){
		        		data.criterion = data.criterion1 + "|" + data.criterion2;
		        		data.criterion1 = "";
		        		data.criterion2 = "";
		        	}
		        	
		        	this.model.set(data);
		        	this.view.filterCollection.add(this.model, {merge:true});
		        	
		        	data = '"filter":' +JSON.stringify(this.view.filterCollection);
		        	console.log(data);
		        	
		        	//this.model.save(data); not necessary
		        	this.view.trigger("submitFilter", this.$el, data);
		        	
		        },
		        onClearFilter: function(){
		        	
	            	var gridfilterModel = Backbone.Model.extend({
	            	...
	            	var gridfilterModelObj = new gridfilterModel({
	            	...
	            	// Remove sort and filter applied --this.view.filterCollection.remove(this.model);//
	            	this.view.filterCollection.remove(
	            			this.view.filterCollection.where({column: this.model.get("column")}));

	            	//reset sort order//
	            	var sortNumber = 0;
	            	var sortCollection = new Backbone.Collection();
	            	_.each(this.view.filterCollection.models, function (filter) {
	            		
		     			  if(filter.get("operator") == "asc" || filter.get("operator") == "desc"){
		     				  filter.set("criterion", ++sortNumber); }
		     			  else
		     				  console.log("No sort");
	        		});
	            	this.view.sortNumber = sortNumber;
	            	this.model = gridfilterModelObj;
	            	
	            	//this.$el.empty();
		        	//this.render();
	            	//above 2 lines makes form unstable, using instead call below..
	            	
		        	this.view.trigger("clearFilter",this, this.model);
		            //reset model. mv 
		        },
	//	        modelChange: function(){
	//	            this.render();
	//	        },
		        onSortFilter: function(e){
		        	//Remove existing sort model and add new sort model.. TODO: Modify to update instead.. mv
	            	var gridSortModel = Backbone.Model.extend({});			            	
            	    var gridSortModelObj, gridSortModelObjOld ;

	            	gridSortModelObj = new gridSortModel({
	            		column : this.model.get("column"),
	            		type : this.model.get("type"),
	            		operator: "asc",
	            		criterion: "1",
	            	...
	            	_.each(this.view.filterCollection.models, function (filter) {
		     			  ...
		     			  if(filter.get("operator") == "asc" || filter.get("operator") == "desc"){
		     				  sortCollection.add(filter);}
	        		});
		   
		    	  gridSortModelObjOld = sortCollection.findWhere({column: this.model.get("column")});
		    	  
		        	if(this.$el.find(".icon-chevron-up").length > 0){  //desc          	
		        		this.$el.find(".icon-chevron-up").removeClass("icon-chevron-up")
		        		.addClass("icon-chevron-down");
		        				            	
		        		gridSortModelObj.set("operator","desc");
		            			            	
		        	}
		        	else if(this.$el.find(".icon-chevron-down").length > 0){
		        		this.$el.find(".icon-chevron-down").removeClass("icon-chevron-down")
		        		.addClass("icon-chevron-up");		
		        				            	
		        		gridSortModelObj.set("operator","asc");
				            	
		        	}
		        	//this.model.set(data);

		        	if (gridSortModelObjOld !== undefined){
		        		gridSortModelObj.set("criterion", gridSortModelObjOld.get("criterion"));
		        		this.view.filterCollection.remove(gridSortModelObjOld);
		        	}
		        	else{//TODO: need to create a separate sort collection
		        		gridSortModelObj.set("criterion", ++this.view.sortNumber);
		        	}
		        	
		        	this.view.filterCollection.add(gridSortModelObj);
		        ...
		        render: function () {
		        	//var html = Backbone.Marionette.Renderer.render(gridfilterTpl, this.model);
		            this.$el.html(this.template(this.model.attributes));
		            return this.$el;
		        }
		    });
	
		    bbGrid.FilterView2.extend = Backbone.View.extend;
		    
		    bbGrid.TheadView2 = function (options) {
		        this.events = {
		            'click th.rdg-column': 'onColumnClick',
		            //'click th.rdg-select' : 'onSort',
		            'click th.rdg-select input[type=checkbox]': 'onAllCheckbox'
		        };
		        Backbone.View.apply(this, [options]);
		        this.view = options.view;
		        this.view.filterCollection = new Backbone.Collection();
		        this.view.sortNumber = 0;
		    };
	
		    _.extend(bbGrid.TheadView2.prototype, Backbone.View.prototype, {
		        tagName: 'thead',
		        ...
		        onColumnClick: function(event){
		        	$(event.target).find(".rdg-sort").show().click();
		        },
		        ...
		        render: function () {
		            ...
		            if (!this.$headHolder) {
			            ...
			            if (this.view.multiselect == 1){
			            	that.$headHolder.append("<th class='rdg-select' title='"+"'>" 
			            			+"<input type='checkbox'>" //mv-select all disabled for now.  will only check rows currently in grid. kj turn back on needs work
			            			+"</th>");			            	
			            ...
			            _.each(cols, function (col) {
			            	...
			            	var gridfilterModelObj = new gridfilterModel({
			            		...
			            		column : col.name,
			            		type : (col.type != undefined && col.type != null) ? col.type : "string",
			            		operator: "equals",
			            		criterion: "",
			            		criterion1: "",
			            		criterion2: ""
			            	...
			            	that.view.FilterView2 = new that.view.entities.FilterView({ view: that.view, model: gridfilterModelObj });
			            	that.view.$FilterView2 = that.view.FilterView2.render();

			            	var thWidth = "";
			            	if(col.width){
	            				thWidth += " style='width:"+col.width +"'";
	            			}
			            	
			            	that.$headHolder.append("<th "+ thWidth +"class='rdg-column' title='"+col.title+"'>"
			            			+col.title+"</th>");
			            	that.$headHolder.find("th[title='"+col.title+"']").append(that.view.$FilterView2);
			            	that.view.FilterView2 = null;
			            	that.view.$FilterView2 = null;
			            });
		    ...
		    bbGrid.TheadView2.extend = Backbone.View.extend;
			    ...
			    _.extend(bbGrid.TheadView.prototype, Backbone.View.prototype, {
			        ...
			        template: _.template(
			            ...
			            <%} if (isContainSubgrid) {%>\
			                <th style="width:15px"/>\
			                <%} _.each(cols, function (col) {%>\
			                    <th <%if (col.width) {%>style="width:<%=col.width%>"<%}%>><%=col.title%><div <% \
			                        if (col.sortOrder === "asc" ) {%>class="icon-chevron-up"<%} else \
			                            if (col.sortOrder === "desc" ) {%>class="icon-chevron-down"<% } %>/><i></th>\
			            <%})%>', null, templateSettings
			        ),
			        onAllCheckbox: function (event) {
			            this.view.trigger('checkall', event);
			        ...
			        onSort: function (event) {
			        	...
			        	if(!this.view.enableServerFilter){
			        		event.stopPropagation();
			        	}else{
			        		this.view.trigger('sort', event);	
			        	}
	    ...
	    _.extend(bbGrid.FilterView.prototype, Backbone.View.prototype, {
	                ...
	                <td>\
	                    <%if (col.filter) {%>\
	                        <<% if (col.filterType === "input") \
	                            {%>input<%}else{%>select<%\
	                            }%> id="<%if (col.filterColName) {%><%=col.filterColName%><%}else{%><%=col.name %><%}%>" \
	                            class="form-control" name="filter" type="text" value="<%=filterOptions[col.name]%>">\
	                    <% if (col.filterType !== "input") {%>\
	        ...
	        onFilter: function () {
	            ...
	            _.each($('*[name=filter]', this.$el), function (el) {
	                text = $.trim($(el).val());
	                if (text) {
	                    self.view.filterOptions[el.id] = text;
	                } else {
	                    delete self.view.filterOptions[el.id];
	                }
	            });
	            if (!this.view.loadDynamic) {
	    ...
	    bbGrid.View = function (options) {
	        ...
	        if (this.enableServerFilter == true){
		        this.entities.FilterView = bbGrid.FilterView2;
		        this.entities.TheadView = bbGrid.TheadView2;
	        }
//	        if (this.showTooltipOnLargeContent == undefined || this.showTooltipOnLargeContent == true){
//	        	this.entites.RowView = bbGrid.RowView2;
//	        }
	        ...
	        //this.render();  //mv - render will be called from view call
	        if (this.autofetch) {
...
//	        	if (enableScroll){
//		        	return 'bbGrid-container bbGrid-scroll';
//	        	}
//	        	else 
	        		return 'bbGrid-container';
	        ...
	        EventHandler: function (eventName, option1, option2) {
	            ...
	            case 'refresh':
	                ...
	                break;
	            case 'submitFilter':				//added server filter. mv
	            	this.onSubmitGridFilter(option1, option2);
		        	break;
	            case 'clearFilter':				//added server filter. mv
	            	this.onClearGridFilter(option1, option2);
		        	break;		        	
	            default:
	                break;
	            }
	        ...
	        collectionEventHandler: function (eventName, model, collection, options) {
	            ...
	            switch (eventName) {
	            case 'add':
	            	if (this.enableServerFilter == undefined)
	            		this.addModelsHandler(model, collection, options);
	            	else
	            		this.addModelsHandler2(model, collection, options);
	                break;
	            case 'change':
	                if (this.enableFilter && this.filterBar) {
	        ...
	        showCollection: function (collection, clearGrid) {  //add arg to clear grid optionally
	            var self = this;
	            if (clearGrid)
	            	this.clearGrid();
	        ...
	        onCheckAll: function (event) { //work on this area for the selectall/deselectall
	        	//http://stackoverflow.com/questions/386281/how-to-implement-select-all-check-box-in-html
	        	//http://direct-fuel-injection.github.io/bbGrid/
	        ...
	        //leaving original function intact.
	        ...
	        addModelsHandler2: function (model, collection, options) {
	        	var that = this;
	        ...
	        //modified version to not clear grid each page (scroll)
	        renderPage2: function (options) {
	            ...
	            this.showCollection(this.collection.models.slice(_.size(this.rowViews), this.collection.length), false);	//added to not cleargrid mv.   
	        ...
	        onSubmitGridFilter: function ($el, data){  //added for server filtering. mv
	            if (this.onSubmitFilter) {
	            	this.clearGrid();
	                this.onSubmitFilter(data);
	                if ($el != undefined){
		                $el.find(".dropdown").removeClass("open"); //closes form
		                $el.find(".glyphicon-filter").addClass("filtered");   	                	
	                }

	            }
	        },	
	        onClearGridFilter : function(that, model){
	        	// Removes current filterview and adds new filterview. need to redo. mv
	        	defaultModel = model;
	        	that.$el.remove();
                that.$headHolder = $('tr.bbGrid-grid-head-holder');
                	        	
	            cols = _.filter(that.view.colModel, function (col) {return !col.hidden; });
	            cols = _.map(cols, function (col) { col.title = col.title || col.name; return col; });
	        	
	            _.each(cols, function (col) {
	            	
	            	if (col.name == defaultModel.get("column")){
		            	console.log(col);
		            	console.log(defaultModel);
		            	that.view.FilterView2 = new that.view.entities.FilterView({ view: that.view, model: defaultModel });
		            	that.view.$FilterView2 = that.view.FilterView2.render();
            	...
            	var data = '"filter":' +JSON.stringify(that.view.filterCollection);
            	         	
	            this.clearGrid(); //does not reset the collection obj.  only removes markup for rows and row views in DOM.
	            if (this.onClearFilter){

	            	this.onClearFilter(data);
	            	that.$el.find(".glyphicon-filter").removeClass("filtered");  
	            }
	        ...
	        onPageChanged: function (event) {
	            var $el = $(event.currentTarget),
	                ...
	                page;
	            className = className.split(" ", 1)[0]; //MV
